...
type deployerOptions struct {
	...
	EnableCustomNetworking      bool          `flag:"enable-custom-networking" desc:"Configure VPC CNI for custom networking."`
	EnableSecurityGroupsForPods bool          `flag:"enable-security-groups-for-pods" desc:"Configure VPC CNI for Security Groups for Pods and launch nodes in private subnet."`
	...
	VPCCNIImageURI          string            `flag:"vpc-cni-image" desc:"Image to use for the aws-node container of the VPCI CNI daemonset"`
	VPCCNIEnvVars           map[string]string `flag:"vpc-cni-env-vars" desc:"Set environment variables on the VPC CNI"`
}
...
func (d *deployer) Up() error {
	...
	if err := d.addonManager.configureVPCCNI(d); err != nil {
		return err
	}
